                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-inline">
                        <input type="text" name="address" lay-verify="required" maxLength="128" placeholder="请输入关键字进行搜索"
                               autocomplete="off" class="layui-input" id="tipinput" value="${admin.address}">
                        <input type="hidden" name="longitude" id="lonNum" value="${admin.longitude}">
                        <input type="hidden" name="latitude" id="latNum" value="${admin.latitude}">
                        <input type="text" class="layui-input lnglat" id="lnglat" readonly="readonly"
                               value="${admin.longitude?string("0.000000")},${admin.latitude?string("0.000000")}">
    ...
    layui.use(['form'], function () {
        ...
        $("#username").mouseleave(function () {
            ...
            $.ajax({
                url: "checkAdmin",
                type: "get",
                data: {'username': username, 'id': "${admin.id}"},
                cache: false,
        ...
        form.verify({
            remote: function (value) {
                ...
                $.ajax({
                    url: "checkAdmin",
                    type: "get",
                    data: {'username': value, 'id': "${admin.id}"},
                    cache: false,
                    /*必须加上async 才能不能跳转添加*/
                    async: false,
                    success: function (result) {
...
<script>
    ...
    var marker = new AMap.Marker({
        position: new AMap.LngLat("${admin.longitude?string("0.000000")}", "${admin.latitude?string("0.000000")}")   // 经纬度对象，也可以是经纬度构成的一维数组[116.39, 39.9]
    });
    ...
    $().ready(function () {
        /*     var lng="";
             AMap.plugin('AMap.Geolocation', function() {
                 ...
                 geolocation.getCurrentPosition(function(status,result){
                     if(status=='complete'){
                         alert(result.position.toString());
                          lng = result.position.toString();
        ...
        map.setCenter(new AMap.LngLat("${admin.longitude?string("0.000000")}", "${admin.latitude?string("0.000000")}"));
        marker.setMap(map);
    });
